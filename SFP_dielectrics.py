...
def generate_dielectric_tab(filename, minwave, maxwave, dwave, m_in):
    ...
    for iwave in range(nwaves):
        ...
        file.write("%15.8f%15.8f%15.8f\n" %
                   ((wave)/1000.0, np.real(eps_in), np.imag(eps_in)))
    # for
    file.close()
# def
...
def generate_tab(filename, minwave, maxwave, dwave, func_pointer, m=1.00, gscat=0.00):
    ...
    for iwave in range(nwaves):
        ...
        # endif
        file.write("%15.8f%15.8f%15.8f\n" % ((wave/m)/1000.0, np.real(eps), np.imag(eps)))
    # for
    file.close()
...
def Ag_JC(wavelength):
    ...
    l = wavelength
    einf = 1.46932164354844
    einfi = 0.245032510494354
    lp = 137.964321665857
    gp = 74484.2077687798
    a1 = 0.163221158200948
    l1 = 307.954950886876
    g1 = 4277.72063494036
    p1 = -1.70922134985866
    a2 = 0.967277507909697
    l2 = 260.730135593913
    g2 = 1051.4738085748
    p2 = 0.154666427887571
    a3 = 0.615093779453933
    l3 = 236.212760050413
    g3 = 1008.62208663784
    p3 = -1.78230663012765
    ...
    iba = np.exp(cone*p1) / (1.0/l1 - 1.0/l - cone/g1)
    ibb = np.exp(-cone*p1) / (1.0/l1 + 1.0/l + cone/g1)
    ib1 = (a1/l1)*(iba+ibb)  # Interband 1
    iba = np.exp(cone*p2) / (1.0/l2 - 1.0/l - cone/g2)
    ibb = np.exp(-cone*p2) / (1.0/l2 + 1.0/l + cone/g2)
    ib2 = (a2/l2)*(iba+ibb)  # Interband 2
    iba = np.exp(cone*p3) / (1.0/l3 - 1.0/l - cone/g3)
    ibb = np.exp(-cone*p3) / (1.0/l3 + 1.0/l + cone/g3)
...
def Ag_JC_SS(wavelength, gscat):
    ...
    einfi = 0.245032510494354
    lp = 137.964321665857
    gp = evnm/(evnm/74484.2077687798+gscat)
    a1 = 0.163221158200948
    l1 = 307.954950886876
    g1 = 4277.72063494036
    p1 = -1.70922134985866
...
def Ag_JC_GNR(wavelength):
    ...
    tmp = np.real(np.sqrt(np.real(deps_dw)**2 + np.imag(deps_dw)**2))
...
def Ag_LH(wavelength):
    # L is the wavelength and should be in nm.
    l = wavelength
    einf = 0.983048524950333
    einfi = 0.337143329306706
    lp = 145.777666579854
    gp = 17192.158229537
    a1 = 0.153920567777116
    l1 = 306.624437448325
    g1 = 4647.05951655734
    p1 = -1.60205817994344
    a2 = 0.852173299670293
    l2 = 265.13703382702
    g2 = 793.646031672091
    p2 = -0.479756609595443
    czero = complex(0.0, 0.0)
    iba = czero
    ibb = czero
    ib1 = czero
...
def Ag_LH_SS(wavelength, gscat):
    ...
    einfi = 0.337143329306706
    lp = 145.777666579854
    gp = evnm/(evnm/17192.158229537+gscat)
    a1 = 0.153920567777116
    l1 = 306.624437448325
    g1 = 4647.05951655734
    p1 = -1.60205817994344
...
def Au_LH(wavelength):
    ...
    l2 = 463.507318325189
    g2 = 2290.02237298882
    p2 = -0.951356810003336
    a3 = 0.0973282595247586
    l3 = 57.9550132461995
    g3 = 497.421718909885
    p3 = 6.44681517896198
    a4 = -0.789447146394695
    l4 = 156.777953986622
    g4 = 283.587000505755
    p4 = 9.29750037444383
    czero = complex(0.0, 0.0)
    ...
    ibb = np.exp(-cone*p3) / (1.0/l3 + 1.0/l + cone/g3)
    ib3 = (a3/l3)*(iba+ibb)  # Interband 3
    iba = np.exp(cone*p4) / (1.0/l4 - 1.0/l - cone/g4)
    ibb = np.exp(-cone*p4) / (1.0/l4 + 1.0/l + cone/g4)
...
def Au_LH_SS(wavelength, gscat):
    ...
    einfi = 0.474161581212655
    lp = 135.865937688189
    gp = gp = evnm/(evnm/16088.906016254+gscat)
    a1 = -0.789833578871083
    l1 = 314.245873751911
    g1 = 1469.6864723325
    p1 = -9.84007589746256
